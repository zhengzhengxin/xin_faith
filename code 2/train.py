...
def train_i3d_tea(cfg, model1, model2,model_dense,train_loader, optimizer1,optimizer_dense, scheduler1,scheduler_dense, epoch, criterion,state):
    if state == 1:
        ...
        model2.train()
        model_dense.train()
        for idx,(feat_i3d,feat_tea,length,target) in enumerate(train_loader):
            feat_i3d = feat_i3d.cuda()
            feat_tea = feat_tea.cuda()
            target = target.view(-1).cuda()
            optimizer1.zero_grad()
            optimizer_dense.zero_grad()
            output,_ = model1(feat_i3d,length)
            output = output.squeeze(dim=-1)
...
def test_i3d_tea(cfg, model1, model2,model_dense,test_loader, criterion,state,mode='test'):
    ...
    if state ==1:
        with torch.no_grad():
            for idx,(feat_i3d,feat_tea,length,target) in enumerate(test_loader):
                feat_i3d = feat_i3d.cuda()
                feat_tea = feat_tea.cuda()
                target = target.view(-1).cuda()
                output,_ = model1(feat_i3d,length)
...
def train_mult_conv(cfg, model1,model2,model3,train_loader, optimizer1, optimizer2,optimizer3,scheduler, epoch, criterion1,criterion2):
    ...
    for idx,(feat_place,feat_tea,target) in enumerate(train_loader):
        ...
        out = model1(emb_place,emb_tea)
        out= out.view(-1,2)
        loss = criterion1(out,target)+criterion2(emb_place,target.float())+criterion2(emb_tea,target.float())
        train_loss += loss.item()
        loss.backward()
        optimizer1.step()
        optimizer2.step()
...
# ali concat i3d tea vggish with emo label
def train_i3d_tea_vgg_emo(cfg, model1, model2,model_dense,train_loader, optimizer1,optimizer_dense, scheduler1,scheduler_dense, epoch, criterion_vio,criterion_emo,state):
    if state == 1:
        train_loss=0
        model1.train()
        model2.train()
        model_dense.train()
        for idx,(feat_i3d,feat_tea,feat_vgg,length,target_vio,target_emo) in enumerate(train_loader):
            feat_i3d = feat_i3d.cuda()
            feat_tea = feat_tea.cuda()
            feat_vgg = feat_vgg.cuda()
